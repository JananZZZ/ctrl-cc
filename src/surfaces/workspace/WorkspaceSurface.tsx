        ...
        <>
          ...
          <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              ...
              {viewMode === 'terminal' && <TerminalView sessionId={activeTabId} buffer={terminalBuffer} onSend={(data) => { if (!activeTabId) return; RuntimeKernelBridge.writeTerminal({ guiSessionId: activeTabId, data }); }} />}
              {viewMode === 'split' && (<div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}><div style={{ flex: '0 0 50%', borderRight: '1px solid var(--cc-border-strong)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}><TerminalView sessionId={activeTabId} buffer={terminalBuffer} onSend={(data) => { if (!activeTabId) return; RuntimeKernelBridge.writeTerminal({ guiSessionId: activeTabId, data }); }} /></div><div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}><ChatView blocks={chatBlocks} streaming={runtimeSnapshot?.status === 'busy'} /><ComposerBar viewMode="split" sessionRuntimeMode={activeSession?.runtimeMode} disabled={!isComposerEnabled(activeTabId)} disabledReason="runtime" onDisabledClick={() => { if (activeTabId) { RuntimeKernelBridge.startSession({ guiSessionId: activeTabId, projectId: activeSession?.projectId ?? '', cwd: activeSession?.cwd ?? '.', model: 'sonnet', effort: 'medium', permissionMode: 'default' }).catch((e) => setError(`Runtime start failed: ${String(e)}`)); } }} onSend={handleSend} /></div></div>)}
            </div>
