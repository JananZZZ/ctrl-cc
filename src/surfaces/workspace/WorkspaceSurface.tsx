...
export function WorkspaceSurface() {
  ...
  // 稳定的空数组引用，避免 ?? [] 每次创建新引用导致 Zustand 选择器不稳定
  const emptyArr = useMemo(() => [] as never[], []);
  const chatBlocks = useRuntimeKernelStore(
    useCallback((s) => (activeTabId ? (s.chatBlocks[activeTabId] ?? emptyArr) : emptyArr), [activeTabId, emptyArr])
  );
        ...
        <>
          ...
          <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              {viewMode === 'chat' && <><ChatView blocks={chatBlocks} streaming={runtimeSnapshot?.status === 'busy'} rawBuffer={terminalBuffer} /><ComposerBar viewMode="chat" sessionRuntimeMode={activeSession?.runtimeMode} disabled={!isComposerEnabled(activeTabId)} disabledReason="runtime" onDisabledClick={() => { if (activeTabId) { RuntimeKernelBridge.startSession({ guiSessionId: activeTabId, projectId: activeSession?.projectId ?? '', cwd: activeSession?.cwd ?? '.', model: 'sonnet', effort: 'medium', permissionMode: 'default' }).catch((e) => setError(`Runtime start failed: ${String(e)}`)); } }} onSend={handleSend} /></>}
              {viewMode === 'terminal' && <TerminalView sessionId={activeTabId} buffer={terminalBuffer} onSend={(data) => { if (!activeTabId) return; RuntimeKernelBridge.writeTerminal({ guiSessionId: activeTabId, data }).catch((err) => { setError(`Terminal write failed: ${String(err)}`); }); }} />}
              {viewMode === 'split' && (<div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}><div style={{ flex: '0 0 50%', borderRight: '1px solid var(--cc-border-strong)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}><TerminalView sessionId={activeTabId} buffer={terminalBuffer} onSend={(data) => { if (!activeTabId) return; RuntimeKernelBridge.writeTerminal({ guiSessionId: activeTabId, data }).catch((err) => { setError(`Terminal write failed: ${String(err)}`); }); }} /></div><div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}><ChatView blocks={chatBlocks} streaming={runtimeSnapshot?.status === 'busy'} rawBuffer={terminalBuffer} /><ComposerBar viewMode="split" sessionRuntimeMode={activeSession?.runtimeMode} disabled={!isComposerEnabled(activeTabId)} disabledReason="runtime" onDisabledClick={() => { if (activeTabId) { RuntimeKernelBridge.startSession({ guiSessionId: activeTabId, projectId: activeSession?.projectId ?? '', cwd: activeSession?.cwd ?? '.', model: 'sonnet', effort: 'medium', permissionMode: 'default' }).catch((e) => setError(`Runtime start failed: ${String(e)}`)); } }} onSend={handleSend} /></div></div>)}
            </div>
